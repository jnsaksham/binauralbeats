...
var all_files, files, index, master_path, sequence, shifted_path;

var paths;
function gen_seq(files, master_path, shifted_path) {
    var master, paths, shifted;
    paths = list("N");
    for (var fp, _pj_c = 0, _pj_a = files, _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
        fp = _pj_a[_pj_c];
...
index = 0;
files = random.sample(all_files, 2);
...
flowScheduler.add(WelcMsgRoutineBegin());
flowScheduler.add(WelcMsgRoutineEachFrame());
flowScheduler.add(WelcMsgRoutineEnd());
...
async function experimentInit() {
  ...
  // Initialize components for Routine "WelcMsg"
  WelcMsgClock = new util.Clock();
  Welcome_message = new visual.TextStim({
    win: psychoJS.window,
    name: 'Welcome_message',
    text: 'Hi! \n\nWelcome to the Stroop test. \n\nYou will be shown a list of words that are printed in a different or same color than the meaning of the word. You are tasked with naming the color of the word, not the word itself, as fast as you can.\n\nYou will input the answers by your arrow keys. The mapping is shown on the next screen.\n\nPress any key to continue.',
    font: 'Open Sans',
    units: undefined, 
  ...
  instrText = new visual.TextStim({
    win: psychoJS.window,
    name: 'instrText',
    text: 'Remember, choose the colour of the word, ignoring the word.\n\nleft: red\ndown: green\nright: blue\nup: yellow\n\nPress any key to continue',
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 0.05,  wrapWidth: undefined, ori: 0.0,
    color: new util.Color('white'),  opacity: undefined,
  ...
  // Initialize components for Routine "playSong"
  playSongClock = new util.Clock();
  text_4 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_4',
    text: 'Attempt in 5 seconds',
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 0.05,  wrapWidth: undefined, ori: 0.0,
    color: new util.Color('white'),  opacity: undefined,
  ...
  // Initialize components for Routine "endSong"
  endSongClock = new util.Clock();
  Reset = new visual.TextStim({
    win: psychoJS.window,
    name: 'Reset',
    text: '30s before you get ready for the next attempt.',
    font: 'Open Sans',
    units: undefined, 
    pos: [0, 0], height: 0.05,  wrapWidth: undefined, ori: 0.0,
    color: new util.Color('white'),  opacity: undefined,
...
var WelcMsgComponents;
function WelcMsgRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //------Prepare to start Routine 'WelcMsg'-------
    t = 0;
    WelcMsgClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    ...
    // keep track of which components have finished
    WelcMsgComponents = [];
    WelcMsgComponents.push(Welcome_message);
    WelcMsgComponents.push(key_resp_2);
    
    for (const thisComponent of WelcMsgComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function WelcMsgRoutineEachFrame() {
  return async function () {
    //------Loop for each frame of Routine 'WelcMsg'-------
    // get current time
    t = WelcMsgClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
...
function WelcMsgRoutineEnd() {
  return async function () {
    //------Ending Routine 'WelcMsg'-------
    for (const thisComponent of WelcMsgComponents) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    ...
    key_resp_2.stop();
    // the Routine "WelcMsg" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisTrial of trials) {
      ...
      trialsLoopScheduler.add(playSongRoutineBegin(snapshot));
      trialsLoopScheduler.add(playSongRoutineEachFrame());
      trialsLoopScheduler.add(playSongRoutineEnd());
      ...
      trialsLoopScheduler.add(endSongRoutineBegin(snapshot));
      trialsLoopScheduler.add(endSongRoutineEachFrame());
      trialsLoopScheduler.add(endSongRoutineEnd());
...
function loopsLoopBegin(loopsLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisLoop of loops) {
      const snapshot = loops.getSnapshot();
      ...
      loopsLoopScheduler.add(trialRoutineEnd());
      loopsLoopScheduler.add(blank500RoutineBegin(snapshot));
      loopsLoopScheduler.add(blank500RoutineEachFrame());
      loopsLoopScheduler.add(blank500RoutineEnd());
      loopsLoopScheduler.add(endLoopIteration(loopsLoopScheduler, snapshot));
    }
...
var playSongComponents;
function playSongRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //------Prepare to start Routine 'playSong'-------
    t = 0;
    playSongClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    ...
    seq = sequence[index];
    if ((seq !== "N")) {
        background_sound = new sound.Sound(seq);
    ...
    // keep track of which components have finished
    playSongComponents = [];
    playSongComponents.push(text_4);
    
    for (const thisComponent of playSongComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function playSongRoutineEachFrame() {
  return async function () {
    //------Loop for each frame of Routine 'playSong'-------
    // get current time
    t = playSongClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
...
function playSongRoutineEnd() {
  return async function () {
    //------Ending Routine 'playSong'-------
    for (const thisComponent of playSongComponents) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    if (response.status === PsychoJS.Status.STARTED) {
      let theseKeys = response.getKeys({keyList: ['left', 'down', 'right', 'up'], waitRelease: false});
...
var endSongComponents;
function endSongRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //------Prepare to start Routine 'endSong'-------
    t = 0;
    endSongClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    ...
    // keep track of which components have finished
    endSongComponents = [];
    endSongComponents.push(Reset);
    
    for (const thisComponent of endSongComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function endSongRoutineEachFrame() {
  return async function () {
    //------Loop for each frame of Routine 'endSong'-------
    // get current time
    t = endSongClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    
    // *Reset* updates
    if (t >= 0.0 && Reset.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      Reset.tStart = t;  // (not accounting for frame time here)
    ...
    if (Reset.status === PsychoJS.Status.STARTED && t >= frameRemains) {
...
function endSongRoutineEnd() {
  return async function () {
    //------Ending Routine 'endSong'-------
    for (const thisComponent of endSongComponents) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
